...
# ╔═╡ 43afcc8d-5e53-4afd-9753-889b2f7781db
using PlutoUI

# ╔═╡ c5adcdfc-f8f4-4a4f-ad14-93a688b627f0
begin
	using MLJ
	using Zygote
	using Plots
	...
	function trainSVM(xMatrix, yMatrix, epochs,lr, w,b)
	    losses = []
	    for e in 1:epochs
...
# ╔═╡ 19ceda62-dd90-4210-a870-aff23f498dab

@bind yR1 Slider(-1:0.1:2)


# ╔═╡ e67a2ba8-7506-4e7d-ab77-afe09e0e6478

@bind yR2 Slider(-1:0.1:2)


# ╔═╡ cee329a0-fb67-49ab-a4e4-9ec95cdf131e
yR2,yR1

# ╔═╡ 04843ba1-1567-4377-8576-674b071e8967
begin
	
yT = -2:0.1:3
l = [hinge_loss(y, yR1) for y in yT]

plot(yT,l)

l2 = [hinge_loss(y, yR2) for y in yT]

plot!(yT,l2)
end

# ╔═╡ a6f5c83f-9cf6-4582-a15e-060cda3964c3
begin
	nDatapointsSlider = @bind nDatapoints Slider(50:1:1000)
	
	slopeSlide = @bind slope Slider(-10:0.1:10)
	iSlide = @bind intersec Slider(-1:0.1:1)
	expSlide = @bind exponent Slider(1:1:10)
	md"""
	slope: $(slopeSlide) 
	...
	nDatapoints: $(nDatapointsSlider)
	"""
end

# ╔═╡ 5d8ae80d-6bb7-4b18-adce-8057ca4bf0a0
begin
	...
	descLine(x) = slope .*x .^exponent .- intersec
	yMatrix = [y < descLine(x) ? 1 : -1 for (x,y) in zip(xVals,yVals)]
	
	plot(xVals, yVals, c=yMatrix, seriestype=:scatter, 			ylim=(minimum(yVals),maximum(yVals)),legend=false)
	pTarget = plot!(xVals, descLine(xVals), label="Descision Line")
end

# ╔═╡ 1073f2fb-be33-488e-924a-2c2c8fc82cef
begin
	lrScale = map(exp10, -6:0)
lrSlide = @bind lr Slider(lrScale)
	epochSlide = @bind epochs Slider(5:100000)
	md"""
	lr: $(lrSlide) 
	epochs: $(epochSlide) 
	"""
...
# ╔═╡ 39ebf4ef-390c-403d-a6ba-07d9d12d8664
lr

# ╔═╡ 42fc6d76-2e21-4884-86b7-c14aea218ff0
begin
...
losses, w, b= trainSVM(xMatrix, yMatrix, epochs, lr,w,b)
y_pred = svm(xMatrix,w,b)
trained_loss = hinge_loss(y_pred,yMatrix)

	lay = @layout [a ;b]
p1 = plot(1:size(losses)[1], losses, yaxis=:log)
	p2 = plot(1:size(losses)[1], losses)
plot(p1,p2,layout=lay)

end

# ╔═╡ b1077fb0-71b4-4f24-ab28-2f4b70ffebb3
begin
...
correctPreds = [
    x == y ? 1 : -1 for (x,y) in zip(labels_pred, yMatrix)
]

end

# ╔═╡ 37d4ef14-7f82-4202-a010-738537996d0b
makeMarginMap(xVals,yVals,yMatrix,w,b, 300)

# ╔═╡ 56bc6da0-2e3d-4c84-a2b6-3a5edeeb36c0
begin
pPredict = scatter(xMatrix[:,1],xMatrix[:,2],marker_z=convert(Vector{Int64},
	labels_pred),legend=false,colorbar=true,
	show=false,marker = (:o,5),colorbar_title="1 is correct -1 is not",
	ylim=(minimum(yVals),maximum(yVals)))
	""
end

# ╔═╡ 46c4146b-a6e5-409d-a8b8-d7f8717e2d7d
plot(pTarget,pPredict,layout=(1,2))

# ╔═╡ 9f3eaa64-d95e-4a68-bff6-5e881b6398de
begin
	acc = sum(correctPreds)/length(correctPreds)
	"Accuracy is $(floor(Int64,acc*100)) %"
end
